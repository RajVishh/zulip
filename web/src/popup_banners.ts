...
const get_api_rate_limit_exceeded_label = (retry_delay_secs: number): string => {
    ...
    return $t(
        {
            defaultMessage:
                "You've exceeded Zulip's usage limits.{retry_delay_secs, plural, one { Please close any extra Zulip tabs, and try again after {retry_delay_secs} second…} other { Please close any extra Zulip tabs, and try again after  {retry_delay_secs} seconds…}}",
        },
        {retry_delay_secs},
    );
};
